...
BEGIN TRY
	...
	IF (TRIM(ISNULL(@project_code, '')) <> '' 
		AND NOT EXISTS(SELECT TOP 1 1 FROM mas_payment_priority_configs WHERE project_code = @project_code))
	BEGIN
		INSERT INTO mas_payment_priority_configs(
			oid, 
			project_code, 
			ServiceTypeId, 
			priority_order, 
		...
		SELECT 
			...
			@project_code, 
			ServiceTypeId, 
			ROW_NUMBER() OVER (ORDER BY ServiceTypeId) AS priority_order,  -- Tạo thứ tự ưu tiên
			@UserId, 
			GETDATE(), 
			@UserId, 
			GETDATE()
		FROM [dbSHome].[dbo].[MAS_ServiceTypes]
		WHERE ServiceType IS NOT NULL  -- Lọc bỏ bản ghi không hợp lệ nếu cần
	END
    ...
    FROM
        ...
        INNER JOIN MAS_ServiceTypes b ON b.ServiceTypeId = a.ServiceTypeId
