...
CREATE procedure [dbo].[sp_Hom_Service_Living_Calculate_Covid]
	...
	begin
		UPDATE t
		 ...
		 FROM [MAS_Service_Living_CalSheet] t
			...
			join MAS_Projects mp on b.ProjectCd = mp.sub_projectCd or b.ProjectCd = mp.projectCd
		...
		INSERT INTO [MAS_Service_Living_CalSheet]
			...
			UPDATE b
			SET IsCalculate = 1
				  ,Amount = (Select sum(Amount) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId) *(1 - (case when @LivingType = 1 then (case  mp.type_discount_elec when 1 then 0.1  
							 ...
							 when 0 then 1 end) else 0 end))
				  ,freeAmt =  (Select sum(isnull(freeAmt,0)) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId)
				  ,DiscountAmt = (Select sum(Amount) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId)*(case when @LivingType = 1 then (case  mp.type_discount_elec when 1 then 0.1  
		                     when 2 then (case when (Select sum(Quantity) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId) > 200 then 0.1 else 0.15 end)
							 when 0 then 1 end) else 0 end)
			FROM MAS_Service_Living_Tracking b 
			...
			UPDATE b
			SET IsCalculate = 1
				  ,Amount = (Select sum(Amount) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId) *(1 - (case when @LivingType = 2 then (case  mp.type_discount_water when 1 then 0.15  
							 when 0 then 1 end) else 0 end))
				  ,freeAmt =  (Select sum(isnull(freeAmt,0)) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId)
				  ,DiscountAmt = (Select sum(Amount) from [MAS_Service_Living_CalSheet] where TrackingId = b.TrackingId)*(case when @LivingType = 2 then (case  mp.type_discount_water when 1 then 0.15
							 when 0 then 1 end) else 0 end)
			FROM MAS_Service_Living_Tracking b 
