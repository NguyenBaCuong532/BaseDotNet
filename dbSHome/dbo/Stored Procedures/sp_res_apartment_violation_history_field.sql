CREATE OR ALTER PROCEDURE [dbo].[sp_res_apartment_violation_history_field]
...
BEGIN TRY
    ...
    BEGIN
        ...
        BEGIN
            SELECT 
                ...
                ApartmentId = NULL,
                apartOid = NULL,
                tableKey = N'apartment_violation',
    ...
    -- Khai báo biến
    DECLARE @tableKey NVARCHAR(100) = N'apartment_violation';
    ...
    SELECT
        ...
        , columnValue = ISNULL(
            CASE a.field_name
                WHEN 'Id' THEN LOWER(CONVERT(NVARCHAR(100), b.Id))
                WHEN 'ViolationContent' THEN b.Content
                WHEN 'ViolationDate' THEN CONVERT(NVARCHAR(10), ISNULL(b.ViolationDate, @ApartmentId), 103)
...
BEGIN CATCH
    ...
    SET @ErrorMsg = 'sp_res_apartment_violation_field' + ERROR_MESSAGE();
