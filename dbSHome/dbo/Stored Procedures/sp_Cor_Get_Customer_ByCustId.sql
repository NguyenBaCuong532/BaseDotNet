...
CREATE procedure [dbo].[sp_Cor_Get_Customer_ByCustId]
@userId	nvarchar(450),
@custId	nvarchar(50)
...
as
	...
	--1
		SELECT a.CustId 
			  ...
			  ,STUFF((
				  ...
				  FROM MAS_Category_Customer aa 
					join MAS_Category b
					 on aa.CategoryCd = b.CategoryCd
				  WHERE aa.CustId = a.CustId 
	  ...
	  --2
	   SELECT [CategoryCd]
			  ...
			  ,CategoryLevel
		  FROM [MAS_Category] a
		WHERE exists(SELECT CategoryCd FROM [MAS_Category_Customer] 
			WHERE CategoryCd = a.CategoryCd and CustId = @custId)
	...
	begin
	  ...
	  FROM MAS_Customers a 
		  left join MAS_Points p on a.CustId = p.CustId
		  left join MAS_Contacts cc on a.CustId = cc.CustId 
	  WHERE exists(select userId from users where CustId = a.CustId and UserId = @userId)
	  --2
		SELECT [CategoryCd]
			  ,[CategoryName]
			  ,case when CategoryLevel = 0 then [CategoryName] else '--' + [CategoryName] end as [ShowName]
		...
		WHERE exists(SELECT CategoryCd FROM [MAS_Category_Customer] b
			...
			WHERE CategoryCd = a.CategoryCd and c.UserId = @userId)
	...
	begin catch
		...
		set @ErrorMsg					= 'sp_Get_Customer_ByCustId ' + error_message()
