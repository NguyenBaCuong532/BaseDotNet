...
-- Description:	Lấy thông tin field cho form MAS_Apartments
-- Output: 3 result sets (Info, Groups, Data)
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[sp_res_apartment_field]
...
BEGIN TRY
    ...
    -- Khai báo biến
    DECLARE @tableKey NVARCHAR(100) = N'MAS_Apartments';
    ...
    IF @ApartmentId IS NOT NULL OR @Oid IS NOT NULL
    BEGIN
        INSERT INTO #tempIn
        SELECT *
        FROM MAS_Apartments
        WHERE ((@ApartmentId IS NOT NULL AND ApartmentId = @ApartmentId)
           OR (@Oid IS NOT NULL AND oid = @Oid))
          AND (@tenantOid IS NULL OR tenant_oid = @tenantOid);
    END
    ELSE
    BEGIN
        ...
        INSERT INTO #tempIn (oid, ApartmentId, RoomCode, tenant_oid)
        VALUES (@newOid, NULL, NULL, @tenantOid);
        SET @Oid = @newOid;
    END
