CREATE procedure [dbo].[sp_spk_shift_log_filter]
	 @UserId nvarchar(450) = NULL
  ,@AcceptLanguage nvarchar(200) = 'vi'
as
begin try
    declare @projectCd nvarchar(50)
    DECLARE @tableKey nvarchar(200) = 'config_sp_spk_shift_log_filter'
		...
		SELECT a.[id]
			  ...
			  ,columnValue = isnull(l.columnValue, 
								case field_name when 'projectCd' then @projectCd
									when 'FromDate' then format(dateadd(day, -2,getdate()),'dd/MM/yyyy')
									when 'ToDate' then format(dateadd(day, 0,getdate()),'dd/MM/yyyy')
