...
-- Description: Cấu hình giá dịch vụ - Gửi xe ngày - Lấy thông tin thêm mới hoặc chỉnh sửa
-- =============================================
CREATE PROCEDURE [dbo].[sp_res_service_price_vehicle_daily_field]
...
AS
BEGIN TRY
    DECLARE @GroupKey NVARCHAR(100) = 'config_sp_res_service_price_vehicle_daily_field_group';--sys_config_data
    DECLARE @TableName NVARCHAR(100) = 'config_sp_res_service_price_vehicle_daily_field';--sys_config_form
    ...
    SELECT
        CASE [data_type]
            WHEN 'uniqueidentifier'
             ...
             WHEN 'bit'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
                        WHEN 'is_active' THEN IIF(b.is_active = 1 OR b.oid IS NULL, 'true', 'false')
                    END)
             WHEN 'datetime'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
        ...
        [isSpecial],
        [isRequire],
--         [isDisable] = IIF([field_name] = 'par_vehicle_type_oid' AND b.Oid IS NOT NULL, 1, [isDisable]),
        [isDisable],
        [IsVisiable],
        [IsEmpty],
    ...
    FROM
        ...
        OUTER APPLY (SELECT TOP 1 * FROM par_vehicle_daily d WHERE oid = @Oid) b
