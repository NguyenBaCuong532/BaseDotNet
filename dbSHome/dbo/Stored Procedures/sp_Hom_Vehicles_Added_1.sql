...
CREATE PROCEDURE [dbo].[sp_Hom_Vehicles_Added]
	@userId				NVARCHAR(450),
...
AS
				...
				(
                    SELECT
                        ...
                        WHERE (@projectCd IS NULL OR d.[projectCd] = @projectCd)
                            ...
                            AND a.[StartTime] BETWEEN @StartDt AND @EndDt 
                            AND (@Filter IS NULL
	...
	begin catch
		declare	@ErrorNum				int = error_number(),
				@ErrorMsg				varchar(200) = 'sp_Hom_Vehicles_Added ' + error_message(),
				@ErrorProc				varchar(50) = error_procedure(),

				@SessionID				int,
				@AddlInfo				varchar(max) = ' - @userId ' + @userId

		exec utl_Insert_ErrorLog @ErrorNum, @ErrorMsg, @ErrorProc, 'sp_Hom_Vehicles_Added', 'GET', @SessionID, @AddlInfo
	end catch