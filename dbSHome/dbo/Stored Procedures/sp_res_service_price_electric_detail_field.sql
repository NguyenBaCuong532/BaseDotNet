...
-- =============================================
CREATE PROCEDURE [dbo].[sp_res_service_price_electric_detail_field]
...
AS
BEGIN TRY
    DECLARE @GroupKey NVARCHAR(100) = 'config_sp_res_service_price_electric_detail_field_group';--sys_config_data
    DECLARE @TableName NVARCHAR(100) = 'config_sp_res_service_price_electric_detail_field';--sys_config_form
    ...
    DECLARE @max_sort_order INT = (SELECT TOP 1 sort_order
                                  ...
                                  WHERE
                                      ...
                                      AND (@par_service_price_type_oid IS NULL OR par_service_price_type_oid = @par_service_price_type_oid)
        ...
        CASE [data_type]
            WHEN 'uniqueidentifier'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
                        WHEN 'oid' THEN b.oid
                        WHEN 'par_electric_oid' THEN ISNULL(@par_electric_oid, b.par_electric_oid)
                        WHEN 'par_service_price_type_oid' THEN ISNULL(@par_service_price_type_oid, b.par_service_price_type_oid)
                    END)
             WHEN 'nvarchar'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
...
--         [IsVisiable],
    ...
    FROM
        ...
        OUTER APPLY (SELECT TOP 1 * FROM par_electric_detail d WHERE oid = @oid) b
        LEFT JOIN par_service_price_type c ON c.oid = ISNULL(@par_service_price_type_oid, b.par_service_price_type_oid)
