...
CREATE procedure [dbo].[sp_Hom_Apartment_Fields]
	@UserId nvarchar(450),
	@apartmentId	bigint
...
as
	...
	--1 profile
	  ...
	  SELECT CardVehicleId
		  ...
		  --,ServiceName
		  ,a.[Status]
		  --,case when c.IsLock = 1 then N'Đã khóa' else case a.[Status] when 0 then N'Chờ phê duyệt' when 1 then N'Đang hoạt động'  when 2 then N'Qúa hạn' else N'Đã khóa' end end [StatusName]
		  --,c.IsLock 
		  ,d.[CardCd]
		  ,a.isVehicleNone
	...
	begin catch
		...
		set @ErrorMsg					= 'sp_Hou_Apartment_Fields ' + error_message()
		set @ErrorProc					= error_procedure()

		set @AddlInfo					= ' '

		exec utl_Insert_ErrorLog @ErrorNum, @ErrorMsg, @ErrorProc, 'ApartmentField', 'GET', @SessionID, @AddlInfo
	end catch