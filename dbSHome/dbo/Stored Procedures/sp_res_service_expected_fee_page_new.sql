...
BEGIN TRY
	...
	SELECT 
		...
		-- Tổng tiền tính theo thời gian & diện tích (đã gồm VAT)
TotalAmt = CASE 
    WHEN a.IsFree = 0 AND a.FreeToDt > msr.ToDt THEN 0
    ELSE 
        ISNULL(a.WaterwayArea, 0) * ISNULL(pc.value, 0) *
        (
            1.0 * (
                DATEDIFF(
                    DAY,
                    ...
                    END,
                    CASE 
                        WHEN ISNULL(pc.expiry_date, msr.ToDt) > msr.ToDt THEN msr.ToDt
                        ELSE ISNULL(pc.expiry_date, msr.ToDt)
                    END
                ) + 1
            ) / NULLIF(DAY(EOMONTH(msr.ToDt)),0)
        )
        * (1 + ISNULL(pc.tax_percent, 0) / 100.0)
END,
...
-- Tiền VAT
FeeVatAmt = CASE 
    WHEN ISNULL(pc.tax_percent, 0) = 0 THEN 0
    ELSE 
        (
            ISNULL(a.WaterwayArea, 0) * ISNULL(pc.value, 0) *
            (
