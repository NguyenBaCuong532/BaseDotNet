CREATE PROCEDURE [dbo].[sp_service_living_meter_set]
    @UserID NVARCHAR(450),
...
BEGIN TRY
    ...
    -- Check IsBill constraint
    DECLARE @ApartmentIdCheck INT;
    SELECT @ApartmentIdCheck = ApartmentId FROM MAS_Apartment_Service_Living WHERE LivingId = @LivingId;

    IF EXISTS (
        SELECT 1 FROM MAS_Service_ReceiveEntry
        WHERE ApartmentId = @ApartmentIdCheck
        AND MONTH(ToDt) = MONTH(CONVERT(DATETIME, @ToDate, 103))
        AND YEAR(ToDt) = YEAR(CONVERT(DATETIME, @ToDate, 103))
        AND IsBill = 1
    )
    BEGIN
