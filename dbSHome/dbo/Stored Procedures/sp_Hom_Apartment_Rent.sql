...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Hom_Apartment_Rent]
	@userId				NVARCHAR(450),
...
AS
				...
				(
                    SELECT 
                        ...
                        WHERE  d.[IsRent] = 1
                            ...
                            AND d.[FeeStart] BETWEEN @StartDt AND @EndDt
							AND (@Filter IS NULL
	...
	begin catch
		declare	@ErrorNum				int = error_number(),
				@ErrorMsg				varchar(200) = 'sp_Hom_Apartment_Rent ' + error_message(),
				@ErrorProc				varchar(50) = error_procedure(),

				@SessionID				int,
				@AddlInfo				varchar(max) = ' - @userId ' + @userId

		exec utl_Insert_ErrorLog @ErrorNum, @ErrorMsg, @ErrorProc, 'sp_Hom_Apartment_Rent', 'GET', @SessionID, @AddlInfo
	end catch