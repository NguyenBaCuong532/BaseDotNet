...
CREATE procedure [dbo].[sp_Pay_Point_Transaction_Page1]
	@UserId	nvarchar(450), 
...
as
	...
	begin try 
		...
		if @Offset = 0
		begin
			select * from dbo.[fn_config_list_gets] ('view_Crm_Get_Point_Trans_Page', @gridWidth - 100) 
			order by [ordinal]
		end
		...
		--1
		  select wa.Ref_No as tranNo
				...
				--		--join UserInfo b on a.UserLogin = b.UserLogin
