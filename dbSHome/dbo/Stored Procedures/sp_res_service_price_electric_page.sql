...
-- =============================================
CREATE PROCEDURE [dbo].[sp_res_service_price_electric_page]
...
AS
BEGIN TRY
    DECLARE @ViewGrid NVARCHAR(100) = 'config_sp_res_service_price_electric_page';--sys_config_list
    ...
    SELECT *
    INTO #par_electricPage
    FROM #par_electric
    ORDER BY created_date DESC
    OFFSET @Offset ROWS FETCH NEXT @PageSize ROWS ONLY;
	
    SELECT
        RecordsTotal = (SELECT COUNT(*) FROM #par_electric),
        RecordsFiltered = (SELECT COUNT(*) FROM #par_electricPage),
        GridKey = @ViewGrid
