...
-- Description: Cấu hình giá dịch vụ - Điện - Danh sách dữ liệu phân trang
-- =============================================
CREATE PROCEDURE [dbo].[sp_res_service_price_electric_set]
...
DECLARE @Valid BIT = 1;
BEGIN TRY
    DECLARE @effective_date_value DATETIME = CONVERT(DATE, @effective_date, 103);
    DECLARE @expiry_date_value DATETIME = CONVERT(DATE, @expiry_date, 103);
    
    DECLARE @electric_oid UNIQUEIDENTIFIER = @oid;
    IF(@is_copy = 1)
        SET @oid = NULL;
...
--            ) -- 7
    ...
    IF EXISTS(SELECT TOP 1 1 FROM #par_electric_check_date)
    BEGIN
        SET @Valid = 0;
    ...
    IF NOT EXISTS (SELECT 1 FROM par_electric WHERE Oid = @Oid)
        BEGIN
            SET @Oid = NEWID();
            INSERT INTO par_electric(oid, project_code, par_residence_type_oid, effective_date, expiry_date, note, is_active, created_user, created_date, last_modified_by , last_modified_date, vat)
            VALUES(@oid, @project_code, @par_residence_type_oid, @effective_date_value, @expiry_date_value, @note, @is_active, @UserId, GETDATE(), @UserId, GETDATE(),@vat);
    ...
    BEGIN
        ...
        SELECT NEWID(), @Oid, par_service_price_type_oid, config_name, start_value, end_value, unit_price, sort_order, @UserId, GETDATE(), @UserId, GETDATE()
        FROM par_electric_detail
        WHERE par_electric_oid = @electric_oid
    END
	
    EXEC sp_res_service_price_set_log @UserId, @project_code, @Oid, @actions, 'par_electric';
