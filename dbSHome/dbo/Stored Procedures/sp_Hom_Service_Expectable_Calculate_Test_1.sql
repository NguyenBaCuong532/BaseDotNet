...
--select * from MAS_Apartments where RoomCode = 'R1-2409'

-- exec sp_Hom_Service_Expectable_Calculate_Test null,'02','4952','31/03/2021'
CREATE procedure [dbo].[sp_Hom_Service_Expectable_Calculate_Test]
...
as
		...
		--			and d.PaidAmt = 0
			...
			select @ToDtVehicle
				...
				WHERE 
					...
					and d.IsPayed = 0
					and d.PaidAmt = 0
					and not exists(select ReceivableId from MAS_Service_Receivable where [ServiceTypeId] = 2 and srcId = v.CardVehicleId and ToDt > @ToDtVehicle)
			...
			--		   ,VATAmt = case v.LivingTypeId when 1  then round(v.Amount/10,0) when 2 then round(v.Amount/5,0) end
			--		   ,TotalAmt = case v.LivingTypeId when 1 then round(v.Amount*1.1,0) when 2 then round(v.Amount*1.15,0) end
			--		   ,fromDt = v.FromDt
			--		   ,[ToDt] = v.ToDt
			--		   ,[Quantity] = 1
			--		   ,price = v.Amount
			...
			--select @ToDt
				...
				--	   ,c.LivingTypeName
				--	   ,v.Amount
				--	   ,case v.LivingTypeId when 1 then round(v.Amount/10,0) when 2  then round(v.Amount/5,0) end
				--	   ,case v.LivingTypeId when 1 then round(v.Amount*1.1,0) when 2 then round(v.Amount*1.15,0) end
				...
				--	--and not exists(select ReceivableId from MAS_Service_Receivable where [ServiceTypeId] = 3 and srcId = v.TrackingId)
				--	and v.ToDt > convert(datetime,'2020-30-11',103)
	...
	--select * from utl_Error_Log where TableName ='Receivable'