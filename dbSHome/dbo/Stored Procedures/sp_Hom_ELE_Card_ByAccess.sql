...
CREATE procedure [dbo].[sp_Hom_ELE_Card_ByAccess]
...
as
	...
	begin
		...
		WHERE u.userId = try_cast(@userId as uniqueidentifier)
			and (
				--exists(select e.EmployeeCd from [dbSHRM].[dbo].[Employees] e where e.CustId = u.CustId and e.emp_st = 1)
				--or 
		...
		if @id > 0 and exists(SELECT t.[FloorName]
			  ...
			  WHERE d.HardwareId = @HardwareId 
					and d.IsActived = 1
					and u.userId = try_cast(@userId as uniqueidentifier)
					and (
						--exists(select e.EmployeeCd from [dbSHRM].[dbo].[Employees] e where e.CustId = u.CustId and e.emp_st = 1)
						--or 
						exists(select * from MAS_Apartment_Member m where m.CustId = u.CustId and m.member_st = 1)
								...
								or ((a.CardTypeId = 1 or a.CardTypeId = 3) 
									and (exists(select 1 from MAS_Apartment_Card ac 
										...
										where ac.CardId = a.CardId 
											and ma.IsReceived = 1
											and ma.isFeeStart = 1
											and mr.FloorNo = t.[FloorName] and mb.ProjectCd = t.ProjectCd and mb.BuildingCd = t.buildingCd)
										)
									)
			  ...
			  WHERE d.HardwareId = @HardwareId 
					...
					and (
						...
						exists(select * from MAS_Apartment_Member m 
									join MAS_Apartments ma on m.ApartmentId = ma.ApartmentId 
									join MAS_Rooms mr on ma.RoomCode = mr.RoomCode 
									join MAS_Buildings mb on mr.BuildingCd = mb.BuildingCd 
				...
				and exists(SELECT a.[CardCd]
					...
					WHERE (Card_Num = @Code) 
						...
						and (T.FloorType = 'CC'
							...
							or (exists(select Id from [MAS_Elevator_Card] ec where ec.CardId = a.CardId and ec.CardRole = 3 and ec.ProjectCd = t.ProjectCd and ec.AreaCd = t.AreaCd))
							or (exists(select Id from [MAS_Elevator_Card] ec where ec.CardId = a.CardId and ec.CardRole = 4 and ec.ProjectCd = t.ProjectCd and ec.AreaCd = t.AreaCd and ec.FloorNumber = t.FloorNumber))
							--or (a.CardTypeId = 2 and exists(SELECT [empId] FROM [dbSHRM].[dbo].[Employees] e join [dbSHRM].[dbo].[Organize_Param] o on e.organizeId = o.[organizeId] where o.organizationCd = 'KSF-Invest' and emp_st = 1 and e.custId = a.custId) and t.FloorType = 'KSF')
							) 
						)
			ORDER BY t.[FloorNumber]
