...
--           ('mau_thong_bao_cat_dich_vu', N'Mẫu nội dung thông báo cắt dịch vụ', 'notify', N'')
    ...
    INSERT INTO par_project_config(oid, project_code, config_code, config_name, config_type, config_value, created_by, created_date, last_modified_by, last_modified_date)
    SELECT NEWID(), @project_code, a.config_code, a.config_name, a.config_type, NULL, @UserId, GETDATE(), @UserId, GETDATE()
    FROM
--         @tblProjectConfig a
    ...
    ORDER BY config_type, config_code, created_date DESC
    ...
    FROM
        ...
        OUTER APPLY(SELECT TOP 1 * FROM par_project_config_default d WHERE a.config_code = d.config_code) d
