...
-- Output: 
-- =============================================
CREATE PROCEDURE [dbo].[sp_app_card_lock] @userId UNIQUEIDENTIFIER = NULL
    , @cardCd NVARCHAR(50)
    , @isLock BIT
    , @memberCard BIT = NULL
...
BEGIN TRY
    ...
    DECLARE @valid_status INT = 1
    DECLARE @apartmentId BIGINT
    DECLARE @current_status INT
    DECLARE @current_lock BIT

    SELECT @apartmentId = ApartmentId
        , @cardcustomerId = CustId
        , @current_status = Card_St
        , @current_lock = SelfLock
    FROM MAS_Cards
    WHERE CardCd = @cardCd
    ...
    IF @memberCard = 1
    BEGIN
        IF dbo.fn_apartment_host_check(@apartmentId, @customerId) = 0
        BEGIN
    ...
    UPDATE MAS_Cards
    SET SelfLock = @isLock
        , Card_St = IIF(@isLock = 1, @card_lock_status, @valid_status)
    WHERE CardCd = @cardCd
