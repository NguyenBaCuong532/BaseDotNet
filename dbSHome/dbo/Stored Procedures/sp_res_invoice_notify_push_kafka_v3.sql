CREATE PROCEDURE [dbo].[sp_res_invoice_notify_push_kafka_v3]
(
...
BEGIN TRY
    ...
    WHERE t.tempCd = 'SERVICE_FEE' 
      AND t.projectCd = @projectcode;
    ...
    (
        ...
        subject, content_email, content_sms,
    ...
    SELECT
        ...
        CASE 
            WHEN CHARINDEX('push', ISNULL(t.actionlist,'')) > 0 
                 AND r.userId IS NOT NULL AND r.userId <> '' THEN 1 
            ELSE 4 
        END,

        CASE 
            WHEN CHARINDEX('sms', ISNULL(t.actionlist,'')) > 0 
                 AND dbo.fn_check_phone_vn(r.phone) = 1 THEN 1 
            ELSE 4 
        END,

        CASE 
            WHEN CHARINDEX('email', ISNULL(t.actionlist,'')) > 0 
    ...
    FROM #Recipients r
    JOIN #NotifyData d 
      ON r.sourceId = d.sourceId AND r.custId   = d.custId
    JOIN NotifyTemplate t 
      ON t.tempId = @tempId
...
BEGIN CATCH
    DECLARE 
        ...
        @ErrorMsg NVARCHAR(4000) = 'sp_res_invoice_notify_push_kafka_v3: ' + ERROR_MESSAGE(),
    ...
    EXEC utl_Insert_ErrorLog 
        ...
        'Receivable', 'Bill', @SessionID, @AddlInfo;
