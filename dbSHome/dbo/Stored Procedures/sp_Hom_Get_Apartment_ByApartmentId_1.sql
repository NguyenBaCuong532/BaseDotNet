...
as
	...
	--1 profile (r đã bỏ - dùng a + ef thay MAS_Rooms)
		SELECT a.[ApartmentId]
			  ...
			  ,a.IsRent 
			  ,MemberCount = (Select count(CustId) from MAS_Apartment_Member where (apartOid = a.oid OR (apartOid IS NULL AND ApartmentId = a.ApartmentId)))
			  ,HouseholdCount = (select count(ch.custid) FROM [MAS_Customer_Household] ch join MAS_Apartment_Member am on ch.CustId = am.CustId where (am.apartOid = a.oid OR (am.apartOid IS NULL AND am.ApartmentId = a.ApartmentId)))
			  ,CardCount = (Select count(cc.CardId) from MAS_Apartment_Card cc join MAS_Cards mc on cc.CardId = mc.CardId where (cc.apartOid = a.oid OR (cc.apartOid IS NULL AND cc.ApartmentId = a.ApartmentId))) 
			  ,VehicleCount = (Select count(vh.CardVehicleId) from MAS_Apartment_Card mm 
					...
					inner join MAS_CardVehicle vh on cc.CardId = vh.CardId
						where (mm.apartOid = a.oid OR (mm.apartOid IS NULL AND mm.ApartmentId = a.ApartmentId)) and cc.Card_St < 3) 
