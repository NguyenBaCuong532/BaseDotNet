...
CREATE procedure [dbo].[sp_res_elevator_floor_field_get]
...
as
	begin try
		...
		select t.[FloorNumber] as FloorNumber
						...
						and (T.FloorType = 'CC'
							or ((a.CardTypeId = 1 or a.CardTypeId = 3) 
								and (exists(select 1 from MAS_Apartment_Card ac 
									...
									where ac.CardId = a.CardId and ma.floorNo = t.[FloorName] and mb.ProjectCd = t.ProjectCd and mb.BuildingCd = t.AreaCd)
									)
								)
							...
							or (exists(select Id from [MAS_Elevator_Card] ec where ec.CardId = a.CardId and ec.CardRole = 4 and ec.ProjectCd = t.ProjectCd and ec.AreaCd = ec.AreaCd and ec.FloorNumber = t.FloorNumber))
							) 
						)
