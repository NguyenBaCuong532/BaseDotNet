...
-- =============================================
CREATE PROCEDURE [dbo].[sp_res_service_price_water_field]
...
AS
BEGIN TRY
    DECLARE @GroupKey NVARCHAR(100) = 'config_sp_res_service_price_water_field_group';--sys_config_data
    DECLARE @TableName NVARCHAR(100) = 'config_sp_res_service_price_water_field';--sys_config_form
        ...
        CASE [data_type]
            WHEN 'uniqueidentifier'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
                        ...
                        WHEN 'par_service_price_type_oid' THEN b.par_service_price_type_oid
                    END)
            WHEN 'datetime'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
            ...
            WHEN 'decimal'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
                        WHEN 'vat' THEN b.vat
                        WHEN 'env_protection_tax' THEN b.env_protection_tax
						 WHEN 'environmental_fee' THEN b.environmental_fee
                    END)
            WHEN 'bit'
                THEN CONVERT(NVARCHAR(MAX), 
                    CASE [field_name]
                        WHEN 'is_active' THEN IIF(b.is_active = 1, 'true', 'false')
                        WHEN 'is_copy' THEN IIF(@is_copy = 1, 'true', 'false')
                    END)
            WHEN 'nvarchar'
                THEN CONVERT(NVARCHAR(MAX),
                    CASE [field_name]
    ...
    FROM
        ...
        OUTER APPLY (SELECT TOP 1 * FROM par_water d WHERE oid = @oid) b
        LEFT JOIN par_service_price_type c ON c.oid = b.par_service_price_type_oid
