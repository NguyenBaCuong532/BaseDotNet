CREATE FUNCTION [dbo].[fn_CalcWaterPriceByPerson_ByPolicy_V2]
(
...
(
    ...
    */
    ...
    ArrParWaterOid AS (
          SELECT DISTINCT par_water_oid, start_value, end_value
          FROM Combined_2),
          
    ArrResultDataInsert AS (
          SELECT
              b.par_water_oid,
              x.period_start,
              x.period_end,
          ...
          FROM
              ArrParWaterOid a
              INNER JOIN par_water w ON a.par_water_oid = w.oid
              OUTER APPLY(SELECT TOP 1 * FROM Combined_2 WHERE par_water_oid = a.par_water_oid) x
              INNER JOIN par_water_detail b ON a.par_water_oid = b.par_water_oid AND a.start_value <> b.start_value AND a.end_value <> b.end_value
--           WHERE b.start_value > @TotalWater
          )

    SELECT * FROM Combined_2
    UNION ALL
    SELECT * FROM ArrResultDataInsert
);