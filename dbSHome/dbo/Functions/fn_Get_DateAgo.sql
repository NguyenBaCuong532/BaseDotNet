...
CREATE FUNCTION [dbo].[fn_Get_DateAgo](@givenDate DateTime,@curDate DateTime)
...
BEGIN
  ...
  CASE
    ...
    WHEN DATEDIFF(dd,@givenDate,@curDate) <= 1 THEN N'Hôm nay'
    WHEN DATEDIFF(dd,@givenDate,@curDate) > 1 AND  DATEDIFF(dd,@givenDate,@curDate) < 7 THEN CONVERT(nVarchar,DATEDIFF(dd,@givenDate,@curDate)) + N' ngày trước ' --+ CAST(DATEDIFF(second, @givenDate, @curDate) / 60 / 60 % 24  AS NVARCHAR(50)) + N' giờ trước'
