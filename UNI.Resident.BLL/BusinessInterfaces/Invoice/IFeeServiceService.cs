...
namespace UNI.Resident.BLL.BusinessInterfaces.Invoice
{
    public interface IFeeServiceService
    {
        ...
        Task<ApartmentFeeInfo> GetApartmentFeeInfo(string empId);
        ...
        Task<string> SetServiceReceivableBill(ServiceReceivableBill bill);// Tạo hóa đơn
        Task<string> SetServiceReceivableBillNew(ServiceReceivableBill bill);// Tạo hóa đơn


        ggDriverFileDownload SetReceiveMoneyServiceGDrive(long receiptId);
        //ggDriverFileStream GetServiceReceivableStream(long receiptId, ReportType reportType);
        #endregion
        #region import-reg
        Task<BaseValidate<Stream>> GetLivingImportTemp(int livingTypeId);
